  ...
  class Client #:nodoc: all
    ...
    def initialize(opts)
      [:api_key, :user, :password].each do |key|
        send("#{key}=", opts[key]) if opts.key?(key)
      end
      ...
      scheme    = uri.scheme
      host      = uri.host
      curb      = opts.key?(:use_curb)  ? !!opts[:use_curb] && CURB_FU : CURB_FU
      port      = opts.key?(:base_port) ?   opts[:base_port]           : (scheme == 'https') ? 443 : 80
      ...
      fail Fastly::Unauthorized unless resp.success?
    ...
    def get(path, params = {})
      ...
      fail Fastly::Error, resp.message unless resp.success?
      JSON.parse(resp.body)
    end

    def get_stats(path, params = {})
      content = get(path, params)
      fail Fastly::Error, content['message'] unless content['status'] == 'success'
      content['data']
    end
...
class CurbFu::Response::Base
  def get_fields(key)
    if ( match = @headers.find { |k, _v| k.downcase == key.downcase })
      [match.last].flatten
    else
      []
    end
